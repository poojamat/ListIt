...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (user == null || user.getName() == null) {
            ...
            TextView mAddListText = (TextView) findViewById(R.id.addList_text);
            mAddListText.setText("Welcome " + user.getName());
            SyncServerDataTask syncTask = new SyncServerDataTask(this);
            syncTask.execute();
        }
        if (((Global) getApplicationContext()).get_Token() == null && user != null && user.getName() != null) {
            mTokenTask = new GetTokenTask(user.getName(), user.getPassword(), this);
            mTokenTask.execute();
        }
                ...
                new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int id) {
                        ...
                        if (desc != null && !desc.isEmpty()) {
                            ToDoList toDoLists = new ToDoList(1, desc, "false");
                            toDoLists.SetSyncStatus(0);
                            toDoLists.SetOperationType(Constants.KEY_Insert);
...
//                        return false;
    ...
    public void emailToDoList(View view) {
        ...
        for (ToDoList i : mToDoLists) {
            //  if ((new String("true").equals(i.getStatus())))
            // {
            emailBody += i.getName() + "\n";
            List<ToDoListItems> list = db.getToDoListItemByListId(String.valueOf(i.getId()));
    ...
    private void initializeData() {
        ...
        List<ToDoList> list = db.getAllToDoListsByOperation();
        for (ToDoList i : list) {
            mToDoLists.add(i);
        }
    }
