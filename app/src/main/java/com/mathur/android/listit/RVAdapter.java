...

{
    ...
    private List<ToDoLists> mDatasetToDos;
    ...
    public RVAdapter(List<ToDoLists> toDoLists) {
    ...
    public void addItem(ToDoLists toDoLists) {
        mDatasetToDos.add(toDoLists);
        int index = getItemCount() + 1;
        notifyItemInserted(index);
    }

    public void updateItem(List<ToDoLists> updatedItems) {
    ...
    public void deleteItem(int index, String desc, String status) {
        ...
        final MySQLiteHelper db = new MySQLiteHelper(mContext);
        ToDoLists toDoLists = new ToDoLists(mDatasetToDos.get(index).getId(), desc, status);
